...
window.addEventListener("load", function () {
  ...
  $("test7segButton").addEventListener("click", () =>
    draw7Seg([47, 3, 8, 19], 3),
  )
  ...
  document.addEventListener("visibilitychange", async () => {
    if (
      ...
      screenWakeLockSentinel
        ...
        .catch((err) => {
          console.error(
            `Failed to release wake lock on visibility change: ${err.name}, ${err.message}`,
          )
        })
    } else if (
  ...
  window.addEventListener("beforeunload", () => {
    if (screenWakeLockSentinel !== null) {
      screenWakeLockSentinel.release().catch(() => {}) // Silent catch - no need to log on page unload
      screenWakeLockSentinel = null
    }
  })
...
function handleClickOutside(event) {
  ...
  if (debugDrawer.classList.contains("visible")) {
    ...
    if (
      !debugDrawer.contains(event.target) &&
...
function decode(data) {
  ...
  if (data.length >= 5 && data[2] == 0x02 && data[3] == 0x40) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      if (chunks[i].trim() !== "") {
        $("debugOutput").insertAdjacentText(
          "beforeend",
          `[${timestamp}] ${chunks[i]}`,
        )
        if (i < chunks.length - 1) {
          $("debugOutput").insertAdjacentElement(
            "beforeend",
...
function drawOledDelta(data) {
  ...
  try {
    unpacked = unpack_7to8_rle(packed)
    console.log(
      `first ${first}, len ${len}, delta size ${unpacked.length} as ${packed.length}`,
    )
  } catch (e) {
  ...
  for (let blk = 0; blk < 6; blk++) {
    for (let rstride = 0; rstride < 8; rstride++) {
      ...
      for (let j = 0; j < blk_width; j++) {
        ...
        if (idata > 0) {
          ctx.fillRect(
            offx + j * px_width + indist,
            offy + y * px_height + indist,
            px_width - 2 * indist,
            px_height - 2 * indist,
          )
        }
...
function draw7Seg(digits, dots, customPixelWidth, customPixelHeight) {
  ...
  if (did_oled) {
    ...
    ctx.fillRect(
      offx,
      offy,
      ctx.canvas.width - 2 * offx,
      ctx.canvas.height - 2 * offy,
    )
  }

  // Use color settings for the 7-segment display
  // Default red LED-like colors, but allow for customization
  const activeColor = displaySettings.use7SegCustomColors
    ? displaySettings.foregroundColor
    : "#CC3333"
  const inactiveColor = displaySettings.use7SegCustomColors
...
let testdata = new Uint8Array([
  240, 125, 2, 64, 1, 0, 126, 127, 0, 102, 0, 66, 76, 71, 18, 44, 100, 0, 6, 8,
  112, 36, 8, 6, 0, 126, 8, 16, 16, 32, 126, 0, 68, 2, 67, 126, 68, 2, 2, 0,
  126, 70, 16, 67, 126, 126, 127, 0, 114, 0, 71, 64, 72, 0, 69, 124, 68, 108, 3,
  124, 120, 68, 0, 67, 96, 69, 120, 70, 12, 69, 120, 57, 96, 0, 0, 112, 124, 27,
  28, 124, 112, 0, 68, 0, 69, 124, 69, 76, 5, 124, 120, 48, 68, 0, 69, 124, 68,
  12, 10, 28, 120, 112, 68, 0, 20, 48, 120, 124, 108, 69, 76, 67, 0, 84, 0, 67,
  96, 69, 120, 70, 12, 69, 120, 67, 96, 68, 0, 69, 124, 71, 76, 66, 12, 86, 0,
  69, 124, 68, 12, 10, 28, 120, 112, 70, 0, 69, 124, 70, 108, 66, 12, 70, 0, 69,
  124, 98, 0, 68, 7, 68, 6, 0, 7, 3, 70, 0, 68, 3, 70, 6, 68, 3, 12, 0, 4, 7, 3,
  70, 1, 12, 3, 7, 4, 0, 68, 7, 28, 0, 1, 7, 6, 4, 68, 0, 68, 7, 68, 6, 4, 7, 3,
  1, 68, 0, 66, 2, 70, 6, 4, 7, 3, 1, 86, 0, 68, 3, 70, 6, 68, 3, 70, 0, 68, 7,
  94, 0, 68, 7, 68, 6, 4, 7, 3, 1, 70, 0, 68, 7, 72, 6, 70, 0, 68, 7, 72, 6,
  126, 98, 0, 247,
])
...
function detectScreenChanges() {
  ...
  if (changedPixels > 0) {
    ...
    addDebugMessage(
      `SCREEN CHANGE at ${now}: ${changedPixels} pixels changed (${changedRegions.join(", ")})`,
    )
...
function initDisplaySettings() {
  ...
  try {
    ...
    if (savedSettings) {
      ...
      displaySettings.pixelWidth =
        parsedSettings.pixelWidth || displaySettings.pixelWidth
      displaySettings.pixelHeight =
        parsedSettings.pixelHeight || displaySettings.pixelHeight
      displaySettings.foregroundColor =
        parsedSettings.foregroundColor || displaySettings.foregroundColor
      displaySettings.backgroundColor =
        parsedSettings.backgroundColor || displaySettings.backgroundColor
      displaySettings.use7SegCustomColors =
        parsedSettings.use7SegCustomColors ||
        displaySettings.use7SegCustomColors
...
function handleSettingChange() {
  ...
  if (
    isNaN(pixelWidth) ||
    pixelWidth < displaySettings.minSize ||
    pixelWidth > displaySettings.maxSize
  ) {
    addDebugMessage(
      `Invalid Pixel Width: ${$("pixelWidth").value}. Must be between ${displaySettings.minSize}-${displaySettings.maxSize}.`,
    )
  ...
  if (
    isNaN(pixelHeight) ||
    pixelHeight < displaySettings.minSize ||
    pixelHeight > displaySettings.maxSize
  ) {
    addDebugMessage(
      `Invalid Pixel Height: ${$("pixelHeight").value}. Must be between ${displaySettings.minSize}-${displaySettings.maxSize}.`,
    )
...
function applyDisplaySettings() {
  ...
  if (
    ...
    addDebugMessage(
      `ERROR: Pixel dimensions must be at least ${displaySettings.minSize}`,
    )
    return
  }
  ...
  if (
    ...
    addDebugMessage(
      `ERROR: Pixel dimensions must not exceed ${displaySettings.maxSize}`,
    )
    return
  }
  ...
  addDebugMessage(
    `Applied display settings: ${pixelWidth}×${pixelHeight} pixels, FG: ${foregroundColor}, BG: ${backgroundColor}`,
  )
...
function updateResizeButtonStates() {
  ...
  $("canvasIncreaseButton").disabled =
    currentSize + displaySettings.resizeStep > displaySettings.maxSize

  // Disable the decrease button if at or below min size
  $("canvasDecreaseButton").disabled =
    currentSize - displaySettings.resizeStep < displaySettings.minSize
}
...
function handleFullscreenChange() {
  ...
  if (document.fullscreenElement) {
    ...
    if (isMobile && "wakeLock" in navigator) {
      navigator.wakeLock
        ...
        .catch((err) => {
          ...
          addDebugMessage(
            `ERROR: Could not acquire screen wake lock: ${err.message}`,
          )
        })
    }
    ...
    if (
      fullscreenSettings.previousPixelWidth > 0 &&
      fullscreenSettings.previousPixelHeight > 0
    ) {
